...
class Wavenet_hx(nn.Module):
    ...
    def __init__(self,
        ...
        # 1x1 convolution to create conditional channels
        self.start_cond_conv = nn.Conv1d(in_channels=self.classes,
                                         out_channels=dilation_channels,
                                         kernel_size=1,
                                         bias=bias)
        ...
        for b in range(blocks):
            ...
            for i in range(layers):
                ...
                self.skip_convs.append(nn.Conv1d(in_channels=dilation_channels,
                                                 ...
                                                 kernel_size=1,
                                                 bias=bias))
                # 1x1 conditional conv
                self.filter_cond_convs.append(nn.Conv1d(in_channels=dilation_channels,
                                                 out_channels=dilation_channels,
                                                 kernel_size=1,
                                                 bias=bias))
                
                self.gate_cond_convs.append(nn.Conv1d(in_channels=dilation_channels,
                                                 out_channels=dilation_channels,
                                                 kernel_size=1,
                                                 bias=bias))
    ...
    def forward(self, input, mode="normal"):
        ...
        for i in range(self.blocks * self.layers):
            ...
            filter = self.filter_convs[i](x) 
            filter = filter + self.filter_cond_convs[i](h)[:,:,-filter.size(2):]
            filter = torch.tanh(filter)

            gate = self.gate_convs[i](x)
            gate = gate + self.gate_cond_convs[i](h)[:,:,-gate.size(2):]
            gate = torch.sigmoid(gate)
            x = filter * gate
